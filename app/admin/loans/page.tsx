...
export default function LoansPage() {
  ...
  const handleActivateLoan = (loanId: string) => {
    const updatedLoans = loans.map((loan) => {
      if (loan.id === loanId) {
        const updated = {
          ...loan,
  ...
  const totalRequestedAmount = loans.reduce((sum, l) => sum + l.amount, 0);
    ...
    <div className="flex h-screen bg-background">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="p-6 sm:p-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-foreground">Loan Requests</h1>
            <p className="mt-2 text-foreground/70">Review and manage loan applications</p>
          </div>

          {/* Summary Stats */}
          <div className="mb-8 grid gap-4 sm:grid-cols-3">
            <Card className="p-6">
              <h3 className="text-sm font-medium text-foreground/70">Pending Requests</h3>
              <p className="mt-2 text-3xl font-bold text-primary">{pendingLoans.length}</p>
            </Card>

            <Card className="p-6">
              <h3 className="text-sm font-medium text-foreground/70">Approved Loans</h3>
              <p className="mt-2 text-3xl font-bold text-secondary">{approvedLoans.length}</p>
            </Card>

            <Card className="p-6">
              <h3 className="text-sm font-medium text-foreground/70">Total Requested</h3>
          ...
          {/* Loans Table */}
          <Card className="p-6">
            <h2 className="mb-6 text-lg font-bold text-foreground">All Loan Requests</h2>

            {loans.length === 0 ? (
              <p className="text-foreground/70">No loan requests yet</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="px-4 py-3 text-left text-foreground/70">Borrower</th>
                      <th className="px-4 py-3 text-right text-foreground/70">Amount</th>
                      <th className="px-4 py-3 text-left text-foreground/70">Purpose</th>
                  ...
                  <tbody>
                    {loans.map((loan) => {
                      const borrower = getUsers().find((u) => u.id === loan.userId);
                      const colors = statusColors[loan.status] || statusColors.pending;
                      return (
                        <tr key={loan.id} className="border-b hover:bg-muted/50">
                          <td className="px-4 py-3">{loan.requestDate}</td>
                          <td className="px-4 py-3 font-medium">{borrower?.name || 'Unknown'}</td>
